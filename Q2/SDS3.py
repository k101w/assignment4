...
class SDS:
    ...
    def sds_loss(
        ...
        with torch.no_grad():
            ...
            x_t = torch.sqrt(self.alphas[t]) * latents + torch.sqrt(1 - self.alphas[t]) * eps 
            
            if text_embeddings_uncond is not None and guidance_scale != 1:
            #     ### YOUR CODE HERE ###
                eps_hat_cond = self.unet(x_t, t, text_embeddings)["sample"]
                eps_hat_uncond = self.unet(x_t, t, text_embeddings_uncond)["sample"]
                eps_hat = eps_hat_uncond + guidance_scale * (eps_hat_cond - eps_hat_uncond)

            else:
                eps_hat = self.unet(x_t, t, text_embeddings)["sample"]
 
        # Compute SDS loss
        w = 1 - self.alphas[t]
        ### YOUR CODE HERE ###
        with torch.no_grad():
            grad = grad_scale * w * (eps_hat - eps)
            target = latents - grad
